...
+ 그래프가 fire되게 하는 setDate메소드 안에서 TimeInterval타입의 변수를 생성하고, 저장된 날짜까지의 오차를 저장 DateFormatter 인스턴스 속성에 파라미터를 저장
+ IAxisValueFormatter를 채용한 클래스인 ChartXAxisFormatter 인스턴스의 파리미터에 오차와 포멧을 전달하면  ChartXAxisFormatter
+ 내부의 Delegate Method에서 값이 변환되어 처리
